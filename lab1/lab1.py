...
#
# rule1 = IF( AND( '(?x) is a Norwegian Blue parrot', '(?x) is motionless' ),
#             THEN('(?x) is not dead' ) )
#
...
# family_rules.
family_rules = [
    # "You can assume that every person will be mentioned in a clause stating his gender."
    # same-identity
    IF('male (?x)', THEN('same-identity (?x) (?x)')),
    IF('female (?x)', THEN('same-identity (?x) (?x)')),
    ...
    # brother
    IF(AND('male (?x)', 'parent (?y) (?x)', 'parent (?y) (?z)', NOT('same-identity (?x) (?z)')), THEN('brother (?x) (?z)', 'sibling (?x) (?z)')),
    # sister
    IF(AND('female (?x)', 'parent (?y) (?x)', 'parent (?y) (?z)', NOT('same-identity (?x) (?z)')), THEN('sister (?x) (?z)', 'sibling (?x) (?z)')),

    # cousin
    IF(AND('parent (?a) (?b)', 'parent (?c) (?d)', 'sibling (?c) (?a)', NOT('same-identity (?d) (?b)')), THEN('cousin (?b) (?d)')),

    # grandparent / grandchild
    IF(AND('parent (?x) (?y)', 'parent (?y) (?z)'), THEN('grandparent (?x) (?z)', 'grandchild (?z) (?x)'))
]
...
WHAT_I_FOUND_BORING = 'Nothing - everything is worth learning!'
