...
def do_multiply_sum_with_sum(sum1, sum2):
    expressions = list()
    for i in sum1:
        for j in sum2:
            expressions.append(Product([i, j]))
    return Sum(expressions)

def do_multiply_sum_with_product(sum1, prod1):
    expressions = list()
    for item in sum1:
        v, e = Product([item, prod1]).evaluate()
        e.append(v)
        expressions.append(Product(e))
    return Sum(expressions)

def do_multiply_product_with_product(prod1, prod2):
    val1, exp1 = prod1.evaluate()
    val2, exp2 = prod2.evaluate()
    val = val1*val2
    exp = exp1+exp2
    exp.append(val)
    return Product(exp)
...
def do_multiply(expr1, expr2):
    if isinstance(expr1, Sum) and isinstance(expr2, Sum):
        return do_multiply_sum_with_sum(expr1, expr2)
    elif isinstance(expr1, Sum) and isinstance(expr2, Product):
        return do_multiply_sum_with_product(expr1, expr2)
    elif isinstance(expr1, Product) and isinstance(expr2, Sum):
        return do_multiply_sum_with_product(expr2, expr1)
    else: # product and product
        return do_multiply_product_with_product(expr1, expr2)
